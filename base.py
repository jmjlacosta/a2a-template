...
class A2AAgent(AgentExecutor, ABC):
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        try:
            ...
            # Use the new artifact creation utility
            await self.create_artifact(
                content=response,
                updater=updater,
                artifact_name=f"{self.get_agent_name()} Result",
                artifact_description="Agent processing result",
    ...
    def _detect_content_type(self, content: Any, filename: Optional[str] = None) -> str:
        """
    ...
    def _is_text_content(self, content: Any, filename: Optional[str] = None) -> bool:
        """
    ...
    def create_part_from_content(
        ...
        """
        Create the appropriate Part type based on content.
        
        CRITICAL: Markdown content MUST be sent as TextPart, not FilePart.
        
        Args:
            content: The content to create a Part from
            filename: Optional filename for type detection
            mime_type: Optional explicit MIME type
            
        Returns:
            Appropriate Part object (TextPart, DataPart, or FilePart)
        """
        # Detect MIME type if not provided
        if not mime_type:
            mime_type = self._detect_content_type(content, filename)
        ...
        # Check if content should be text
        if self._is_text_content(content, filename):
            # Convert bytes to string if needed
            if isinstance(content, bytes):
                try:
                    content = content.decode('utf-8')
                except UnicodeDecodeError:
                    # If decode fails, treat as binary file
                    import base64
                    return FilePart(
                        kind="file",
                        file={
            ...
            # Return TextPart for text content
            if isinstance(content, str):
                return TextPart(
                    kind="text",
                    text=content,
                ...
                # JSON-like data as TextPart with JSON string
                return TextPart(
                    kind="text",
        ...
        # Structured data that's not text
        ...
        # Binary content or files
        if isinstance(content, bytes) or (filename and not self._is_text_content(content, filename)):
            import base64
            bytes_content = content if isinstance(content, bytes) else str(content).encode()
            return FilePart(
                kind="file",
                file={
                    "name": filename or "file",
                    "bytes": base64.b64encode(bytes_content).decode('utf-8'),
                    "mimeType": mime_type or "application/octet-stream"
                }
            )
    ...
    async def create_artifact(
        ...
        """
        Create and add a properly formatted A2A Artifact to the task.
        Per A2A spec section 6.7: Artifacts represent tangible outputs.
        
        Args:
            content: The artifact content
            updater: TaskUpdater instance for adding the artifact
            artifact_name: Human-readable name for the artifact
            artifact_description: Human-readable description
            artifact_id: Unique ID within task scope
            mime_type: Explicit MIME type override
            filename: Filename for type detection (especially for .md files)
        ...
        # Create the appropriate Part based on content
        part = self.create_part_from_content(content, filename, mime_type)
        
        # Build artifact metadata
        artifact_metadata = {
            "agent": self.get_agent_name(),
            "timestamp": time.time(),
            "contentType": self._detect_content_type(content, filename)
        }
        ...
        await updater.add_artifact(
            ...
            name=artifact_name or f"{self.get_agent_name()} Output",
            description=artifact_description,
            metadata=artifact_metadata
        )
        
        self.logger.debug(f"Created artifact {artifact_id} with {type(part).__name__}")
    ...
    async def _get_agent_card(self, agent_name_or_url: str) -> "AgentCard":
        """
        Get or resolve an agent card from a name or URL.
        
        Args:
            agent_name_or_url: Either an agent name from registry or a direct URL
        ...
        if agent_name_or_url.startswith(('http://', 'https://')):
            ...
            card_url = f"{agent_name_or_url.rstrip('/')}/.well-known/agent-card.json"
            ...
            async with httpx.AsyncClient() as client:
                try:
                    ...
                    # If we can't fetch the card, create a minimal one
                    self.logger.warning(f"Could not fetch agent card from {card_url}: {e}")
                    from a2a.types import AgentCapabilities
                    return AgentCard(
                        protocolVersion="0.3.0",
                        name=agent_name_or_url,
                        description="Remote agent",
                        version="unknown",
                        url=agent_name_or_url,
                        preferredTransport="JSONRPC",
                        capabilities=AgentCapabilities(
                            streaming=False,
            ...
            from utils.registry import resolve_agent_url
            
            try:
                agent_url = resolve_agent_url(agent_name_or_url)
                # Recursively get card from resolved URL
                return await self._get_agent_card(agent_url)
            except ValueError as e:
                raise ValueError(f"Failed to resolve agent '{agent_name_or_url}' from registry: {e}")
    ...
    async def call_agent(
        ...
        """
        Call another A2A-compliant agent with any type of message using ADK client.
        
        This method uses the official A2A ADK client for full compliance with the
        ...
        import uuid
        from a2a.types import (
            Message, AgentCard, SendMessageRequest, MessageSendParams
        )
        try:
            from a2a.utils.errors import InvalidParamsError
        except ImportError:
        ...
        # Get or resolve agent card
        agent_card = await self._get_agent_card(agent_name_or_url)
        
        # Create httpx client with proper configuration
        headers = {}
        token = os.getenv("HU_TOKEN") or os.getenv("AGENT_TOKEN")
        if token:
            headers["Authorization"] = f"Bearer {token}"
        
        # Use the agent's URL from the card
        ...
        try:
            # Create ADK client directly with httpx client
            from a2a.client import A2AClient as ADKClient
            adk_client = ADKClient(httpx_client, agent_card=agent_card)
            ...
            if isinstance(message, Message):
                ...
                # Already formatted as dict with parts - convert to Message object
                ...
                formatted_message = Message(
                    role="user",
                    parts=[DataPart(kind="data", data=message)],
                    kind="message",
                    messageId=str(uuid.uuid4())
                )
            else:
            ...
            # Create SendMessageRequest for ADK
            request = SendMessageRequest(
                id=str(uuid.uuid4()),
                jsonrpc="2.0",
                method="message/send",
                params=MessageSendParams(message=formatted_message)
            )
            
            # Send message using ADK client
            response = await adk_client.send_message(request)
            
            # Extract result from response
            # ADK returns SendMessageResponse with .root that can be error or success
            if hasattr(response, 'root'):
                response_root = response.root
                # Check if it's an error response
                if hasattr(response_root, 'error'):
                    raise ValueError(f"Agent returned error: {response_root.error}")
                # Extract result from success response
                if hasattr(response_root, 'result'):
                    result = response_root.result
                else:
